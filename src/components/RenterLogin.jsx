...
const RenterLogin = ({ setUser, setUserType }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/user/renterlogin`,
        credentials,
        {
      ...
      if (response.data.securityToken) {
        ...
        showNotification(credentials.email);
  ...
  return (
    <motion.div className="flex justify-center items-center min-h-screen p-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1 }}>
      <motion.div className="w-full max-w-md p-8 bg-white rounded-lg shadow-2xl" initial={{ scale: 0.9 }} animate={{ scale: 1 }} transition={{ duration: 0.8 }}>
        <h2 className="text-2xl font-semibold text-center mb-4 text-gray-800 underline">Renter Login</h2>

        {message && (
          <p className={`text-center mb-4 p-2 rounded-lg ${message.includes("successful") ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600"}`}>
            {message}
          </p>
        )}
